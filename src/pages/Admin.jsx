...
export default function Admin() {
  ...
  // Removed duplicate function declarations and fixed JSX structure
  const handleImageUpload = async (e) => {
    ...
    try {
      // Use the apiPost utility which handles the base URL and error handling
      const data = await apiPost('/api/upload/image', formData);
      console.log('Upload response data:', data);
      
      // Handle Cloudinary response format
      const imageUrl = data.secure_url || data.imageUrl || data.url || data.data?.url;
      if (!imageUrl) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-8" style={{ fontFamily: 'Marcellus SC, serif' }}>Admin Dashboard</h1>
    ...
    {!!message && (
        <div className={`max-w-6xl mx-auto px-8 py-3 text-sm rounded-md ${message.includes('success') ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
          {message}
        </div>
      )}
                ...
                {filteredProducts.map((p) => (
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="hidden text-xs text-gray-500 mt-1">Image not loading</div>
                    </td>
