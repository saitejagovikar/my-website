...
export const sendOTPEmail = async (email, otp, name = 'User') => {
  if (!process.env.RESEND_API_KEY) {
    console.warn('Skipping OTP email - Resend not configured');
    return { success: false, error: 'Email service not configured' };
  }

  try {
    const { data, error } = await resend.emails.send({
      from: 'SLAY Fashion <onboarding@resend.dev>', // Update this to your verified domain
      to: [email],
...
export const sendPasswordResetConfirmation = async (email, name = 'User') => {
  if (!process.env.RESEND_API_KEY) {
    console.warn('Skipping password reset confirmation email - Resend not configured');
